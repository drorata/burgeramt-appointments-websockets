...
try:
    ...
    exception_message = (
        "You must set the BOOKING_TOOL_EMAIL and BOOKING_TOOL_ID environment variables."
    )
...
appointments_url = (
    ...
    f"tag.php?termin=1&anliegen[]={os.environ['BOOKING_ANLIEGEN']}&dienstleisterlist="
    "122210,122217,327316,122219,327312,122227,327314,122231,327346,122243,"
    "327348,122254,122252,329742,122260,329745,122262,329748,122271,327278,"
    "122273,327274,122277,327276,330436,122280,327294,122282,327290,122284,"
    "327292,122291,327270,122285,327266,122286,327264,122296,327268,150230,"
    "329760,122297,327286,122294,327284,122312,329763,122314,329775,122304,"
...
def get_appointments():
    ...
    headers = {
        ...
        "User-Agent": (
            f"Mozilla/5.0 AppointmentBookingTool/1.1 "
            f"(https://github.com/nicbou/burgeramt-appointments-websockets; {email}; {script_id})"
        ),
    ...
    return sorted(
        list(
            set(
                parse_appointment_dates(response_p1.text)
                + parse_appointment_dates(response_p2.text)
            )
        )
    )
...
def parse_appointment_dates(page_content):
    ...
    bookable_cells = BeautifulSoup(
        page_content, "lxml", parse_only=appointment_strainer
    ).find_all("a")
    appointment_dates = []
...
def look_for_appointments():
    ...
    try:
        ...
        delay = 360
        logger.warning(
            f"Got {err.response.status_code} error. Checking in {delay} seconds"
        )
        return {
            "time": datetime_to_json(datetime.now()),
            "status": 502,
        ...
        return {
            "time": datetime_to_json(datetime.now()),
            "status": 500,
            "message": f"An unknown error occurred: {str(err)}",
            "appointmentDates": [],
        }
